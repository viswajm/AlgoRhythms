...
class FermatBenchmark:
    """Benchmarks Fermat primality test."""
    ...
    def __init__(self, cpp_binary: str = None):
        ...
        if cpp_binary is None:
            binary_name = "benchmark_fermat.exe" if platform.system() == "Windows" else "benchmark_fermat"
            cpp_path = detect_cpp_binary(binary_name, "benchmark_fermat")
        else:
    ...
    def run_cpp_benchmark(self, command: str) -> str:
        ...
        try:
            ...
            if platform.system() == "Windows" and self._use_wsl:
                ...
                cmd = f'wsl -e bash -c "cd {cpp_dir} && ./benchmark_fermat {command}"'
    ...
    def benchmark_all(self) -> None:
        ...
        print("="*70)
        print("FERMAT PRIMALITY TEST - COMPREHENSIVE BENCHMARK")
        print("="*70)
        ...
        if data:
            self.save_to_csv("fermat_scaling.csv", data)
        ...
        if data:
            self.save_to_csv("fermat_k_variation.csv", data)
        ...
        print("="*70)
        try:
            from plot_fermat_benchmarks import FermatPlotter
            plotter = FermatPlotter(self.results_dir)
            plotter.plot_all()
    ...
    def benchmark_custom(self, number: int, k: int, iterations: int = 100) -> None:
        ...
        try:
            ...
            if platform.system() == "Windows" and self._use_wsl:
                cpp_dir = to_wsl_path(self.cpp_dir)
                cmd = f'wsl -e bash -c "cd {cpp_dir} && ./benchmark_fermat {number} {k} {iterations}"'
                result = subprocess.run(
                    cmd,
                    capture_output=True,
                    text=True,
...
def main():
    """Main execution."""
    print("Fermat Primality Test - Benchmark Suite")
    print("="*70)
    
    if len(sys.argv) < 2:
        print("\nUsage:")
        print("  python benchmark_fermat.py <command>")
        print("  python benchmark_fermat.py <number> <k> <iterations>")
        print("\nCommands:")
        ...
        print("\nExamples:")
        print("  python benchmark_fermat.py all")
        print("  python benchmark_fermat.py 1000000007 10 100")
        return
    
    benchmark = FermatBenchmark()
