...
/**
 * @brief Benchmark Miller-Rabin test on a single number
 */
void benchmark_single(uint64_t n, int k, int iterations = 100) {
    cout << "\n" << string(70, '=') << "\n";
    cout << "MILLER-RABIN TEST BENCHMARK\n";
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        PrimalityResult res = miller_rabin_test(n, k);
...
/**
 * @brief Benchmark Miller-Rabin across different input sizes
 */
void benchmark_scaling(int k = 10, int iterations = 50) {
    cout << "\n" << string(70, '=') << "\n";
    cout << "MILLER-RABIN TEST SCALING BENCHMARK\n";
...
/**
 * @brief Benchmark Miller-Rabin with varying k values
 */
void benchmark_k_variation(uint64_t n = 1000000007, int iterations = 50) {
    cout << "\n" << string(70, '=') << "\n";
    cout << "MILLER-RABIN TEST: ITERATIONS (k) BENCHMARK\n";
...
/**
 * @brief Benchmark Miller-Rabin on Carmichael numbers
 */
void benchmark_carmichael(int k = 10, int iterations = 50) {
    cout << "\n" << string(70, '=') << "\n";
    cout << "MILLER-RABIN TEST: CARMICHAEL NUMBERS BENCHMARK\n";
    cout << "k=" << k << ", iterations per number=" << iterations << "\n";
    cout << string(70, '=') << "\n";
    ...
    for (const auto& [num, factors] : carmichael_numbers) {
        ...
        // Warmup
        for (int i = 0; i < 3; i++) {
            miller_rabin_test(num, k);
        }

        // Benchmark
        for (int i = 0; i < iterations; i++) {
            auto start = chrono::high_resolution_clock::now();
            PrimalityResult res = miller_rabin_test(num, k);
            auto end = chrono::high_resolution_clock::now();
...
int main(int argc, char* argv[]) {
    cout << "===================================================================\n";
    cout << "MILLER-RABIN PRIMALITY TEST - WALL-CLOCK TIME BENCHMARK\n";
    cout << "===================================================================\n";
    ...
    if (argc == 2) {
        ...
        // Custom: benchmark_miller_rabin <number> <k> <iterations>
